...
PHASE = None  # First working release
...
__version__ = "0.2.0_private_13-20260106-6338141f"
...
def get_base_version():
    ...
    # Example: 0.1.0_main_13-20260104-ad442287 -> 0.1.0
    if "_" in __version__:
...
def get_pip_version():
    ...
    - Main branch: 0.1.0_main_13-20260104-hash -> 0.1.0
    - Dev branch: 0.1.0_dev_13-20260104-hash -> 0.1.0.dev13
    - Feature branch: 0.1.0_feature_13-20260104-hash -> 0.1.0.dev13+feature
    - Alpha/beta: 0.1.0-alpha_main_13... -> 0.1.0a0 (main) or 0.1.0a13 (dev)
    """
    ...
    # Handle phase in base version (e.g., 0.1.0-alpha)
    phase_suffix = ""
    if "-" in base:
        base_parts = base.split("-")
        base = base_parts[0]
        phase = base_parts[1].lower()
        if phase.startswith("alpha"):
            phase_suffix = "a"
        elif phase.startswith("beta"):
            phase_suffix = "b"
        elif phase.startswith("rc"):
            phase_suffix = "rc"

    if branch == "main":
        # Release version
        if phase_suffix:
            return f"{base}{phase_suffix}0"
        return base
    else:
        # Development version
        ...
        if phase_suffix:
            return f"{base}{phase_suffix}{build_num}"
