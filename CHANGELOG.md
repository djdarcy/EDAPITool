...
### Added
- **The observation store** (#22): one SQLite file, `~/edapitool/store.db`, that keeps everything the tool reads from the game and from Frontier, as read. `Market.json`, `Cargo.json` and `Status.json` are archived every time a command reads them, and so is every answer from Frontier's `/profile`, `/market` and `/fleetcarrier`; the same bytes read twice are one row, a changed file is a new row and the old one stays. The market rows are also re-read into two query-friendly tables under the EDDN commodity schema's column names. The store is additive: a command's output does not change with it on or off, a store that cannot be written costs one line on stderr and nothing else, and `ED_NO_STORE=1` turns it off for a run. `construction` is the one command whose data is not kept yet; it reads journal events, which join the store with journal ingest. See `docs/store.md`.
- **`edapitool store verify | backup | rebuild`.** `verify` checks the file is sound and writes nothing; `backup` makes an online copy beside the file, and only after `verify` passes, so a broken store never replaces a good copy; `rebuild` regenerates the derived market tables from the raw rows and never touches the tables that cannot be regenerated. A store made by a newer version is refused by name rather than read by guesswork. The `writes` table (#29) is reserved here, empty, so that a later version's ledger of what the tool painted can never be regenerated by a rebuild.

### Fixed
- **`serve` no longer reports `MarketData unchanged` when you dock somewhere new without a market.** The check that skips an identical publish compared the wrong row of the grid: it kept the timestamp it meant to ignore and dropped the station it meant to compare, so two stations with no commodity market looked the same and the tab kept showing the first. It now compares the data (station, system, market id, and every price and quantity), never the rendered rows.
- **`serve` no longer re-reads journal lines it has already handled.** The journal is written with Windows line endings and the watcher counted them one byte short per line, so after enough events a poll started inside the previous line and delivered its tail again. It now counts the bytes it read. A journal with plain line endings, copied from another machine, is read too.
